...
dir=$(git rev-parse --show-toplevel)        # this dotfiles directory
...
for file in $(git ls-files config/ | sort -u); do
    ...
    [ -f "$HOME/.$file" ] && [ ! -L "$HOME/.$file" ] && mv "$HOME/.$file" "$olddir"
    rm -rf "$HOME/.$file"
    ln -nsf "${dir}/${file}" "$HOME/.$file"
done

# now lets do something completely different for files in the ./share/ directory
for file in figlet-fonts fortunes dict; do
    echo "working on $file..."
    [ -f "$XDG_DATA_HOME/$file" ] && [ ! -L "$XDG_DATA_HOME/$file" ] && mv "$XDG_DATA_HOME/$file" "$olddir"
    rm -rf "$XDG_DATA_HOME/$file"
    ln -nsf "${dir}/share/${file}" "$XDG_DATA_HOME/$file"
done
...
# aaand skootch over that thang
[ ! -d "$XDG_BIN_HOME" ] && mkdir "$XDG_BIN_HOME"
[ ! -f "$XDG_BIN_HOME" ] && ln -s "$dir/bin" "$XDG_BIN_HOME"
